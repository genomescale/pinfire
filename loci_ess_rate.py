...
new_header = header + ["worst_ess_per_hour", "worst_seconds_per_ess"]

combined_filename = "loci_combined.csv"
combined_file = open(combined_filename, "w")
combined_writer = csv.writer(combined_file)
combined_writer.writerow(new_header)

for result in results:
	k = result['n_partitions']
	l = result['loci_per_partition']
	i = result['individuals_per_species']
	r = result['replicate']
	s = result['n_species']

	worst_rate = 0.0
	worst_inverse_rate = 0.0

	for p in range(k):
		base = "%02dr_%02ds_%02di_%02dk_%03dl_%02dp" % (r, s, i, k, l, p)
		...
		if seconds_per_ess > worst_inverse_rate:
			worst_rate = ess_per_hour
			worst_inverse_rate = seconds_per_ess

	new_row = []
	for h in header:
		new_row.append(result[h])

	new_row.append(worst_rate)
	new_row.append(worst_inverse_rate)
