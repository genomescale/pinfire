...
defaults_group.add_argument("-p", "--probability-method", type = str, default="conditional-clade", choices = ["tree-topology", "conditional-clade"], help = "Calculate tree topology probabilities using tree topology frequencies, or using conditional clade frequencies. Default: conditional-clade.")
...
defaults_group.add_argument("-m", "--max-topologies", type = int, default = 1, help = "How many tree topologies (in descending order of posterior probability) to output. Default: 1.")

output_group = arg_parser.add_argument_group('output files')
output_group.add_argument("-n", "--newick-output", metavar = "NEWICK_OUTPUT_PATH", type = str, help = "Output the summary tree(s) in newick format. When -m/--max-topologies is greater than 1, more than one tree may be returned, so an identifying number will be appended to the end of each filename. Default: None.")
